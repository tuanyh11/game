        ...
        for (let chunkStart = 0; chunkStart < this.rows; chunkStart += chunkSize) {
            ...
            for (let r = chunkStart; r < chunkEnd; r++) {
                for (let c = 0; c < this.cols; c++) {
                    ...
                    // ===== BASE FILL with richer palette =====
                    switch (t) {
                        case TerrainType.Grass:
                            ctx.fillStyle = varyColor(52, 128, 28, bh, 24); break;
                        case TerrainType.GrassDark:
                            ctx.fillStyle = varyColor(35, 95, 16, bh, 20); break;
                        case TerrainType.GrassLight:
                            ctx.fillStyle = varyColor(68, 162, 42, bh, 26); break;
                        case TerrainType.GrassFlower:
                            ctx.fillStyle = varyColor(58, 138, 35, bh, 22); break;
                        case TerrainType.Sand:
                            ctx.fillStyle = varyColor(200, 185, 120, bh, 22); break;
                        case TerrainType.Dirt:
                            ctx.fillStyle = varyColor(108, 90, 62, bh, 18); break;
                        case TerrainType.DirtDark:
                            ctx.fillStyle = varyColor(74, 60, 40, bh, 16); break;
                        case TerrainType.Rock:
                            ctx.fillStyle = varyColor(115, 115, 108, bh, 20); break;
                        case TerrainType.Water: {
                            ...
                            const wR = 30 + (1 - depthFactor) * 20;
                            const wG = 115 + (1 - depthFactor) * 30;
                            const wB = 165 + (1 - depthFactor) * 15;
                            ctx.fillStyle = varyColor(wR, wG, wB, bh, 18);
                            break;
                        }
                        case TerrainType.Bridge:
                            ctx.fillStyle = varyColor(110, 82, 45, bh, 16); break;
                    }
                    ctx.fillRect(x, y, TS, TS);

                    // ===== DIRECTIONAL LIGHTING: top-left light source =====
                    // Subtle gradient overlay on every tile
                    ...
                    if (t === TerrainType.Grass) {
                        ...
                        if (h < 0.4) {
                            ...
                            ctx.fillStyle = '#d8cc98'; // highlight (top of ripple)
                            ctx.fillRect(x + 1, rippleY, TS - 2, 1);
                            ctx.fillStyle = '#b0a468'; // shadow (bottom of ripple)
