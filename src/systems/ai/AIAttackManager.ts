...
export function sendAttackWave(ai: AIContext): void {
    ...
    ai.wavesSent++;
    // Cap wave size growth: grow slowly, reset every 3 waves to prevent stalling
    if (ai.wavesSent % 3 === 0) {
...
export function rallyTroops(ai: AIContext): void {
    ...
    for (const u of rallyUnits) {
        ...
        // Ensure units don't constantly shuffle if they are "close enough" (within 5 tiles of rally center)
        if (dist > TILE_SIZE * 5) {
            // Tighter spread radius (0-8 tiles) so units converge faster
            const angle = Math.random() * Math.PI * 2;
            const spreadRadius = Math.random() * TILE_SIZE * 8;
