...
import { cmdMove, cmdAttackUnit, cmdAttackBuilding, cmdGather, cmdBuildAt, cmdBuildPlace, cmdTrain, cmdDeleteUnit, cmdDeleteBuilding, cmdReturnResources } from "../network/NetworkCommands";
...
export class SelectionSystem {
    ...
    private commandIndicators: { x: number; y: number; timer: number; type: 'attack' | 'move' | 'gather' | 'rally'; radius?: number; color?: string }[] = [];
    ...
    /** Which team this player controls (0 for singleplayer, varies in multiplayer) */
    playerTeam = 0;

    /** Multiplayer mode: commands go through network instead of direct execution */
    isMultiplayer = false;

    /** Callback to send command to server (set by Game in multiplayer) */
    sendCommand: ((cmd: GameCommand) => void) | null = null;
    ...
    private handleMinimapRightClick(sx: number, sy: number): void {
        ...
        if (this.isMultiplayer && this.sendCommand) {
            this.sendCommand(cmdMove(this.playerTeam, unitIds, worldX, worldY));
        } else {
            // Move in formation (singleplayer)
            const spacing = 20;
    ...
    private updateCursor(e: MouseEvent): void {
        ...
        if (!this.isPaused) {
            const hasOwnUnits = this.selectedUnits.length > 0 &&
                this.selectedUnits.some(u => u.team === this.playerTeam);
            const hasVillagers = this.selectedUnits.some(u => u.isVillager && u.team === this.playerTeam);
            const hasCombatUnits = hasOwnUnits && this.selectedUnits.some(u => !u.isVillager && u.team === this.playerTeam);
            ...
            if (this.buildMode) {
                ...
                if (hasCombatUnits || hasVillagers) {
                    ...
                    const enemyUnit = this.entityManager.findEnemyUnitAt(world.x, world.y, this.playerTeam, 45);
                    if (enemyUnit) foundEnemy = true;

                    if (!foundEnemy) {
                        const enemyBuilding = this.entityManager.findEnemyBuildingAt(world.x, world.y, this.playerTeam, 25);
    ...
    private onKeyDown(e: KeyboardEvent): void {
        ...
        if (e.key === 'Delete' || e.key === 'Backspace') {
            ...
            const ownUnits = this.selectedUnits.filter(u => u.team === this.playerTeam);
            if (ownUnits.length > 0) {
                if (this.isMultiplayer && this.sendCommand) {
                    this.sendCommand(cmdDeleteUnit(this.playerTeam, ownUnits.map(u => u.id)));
                } else {
            ...
            if (this.selectedBuilding && this.selectedBuilding.team === this.playerTeam && this.selectedBuilding.alive) {
                if (this.isMultiplayer && this.sendCommand) {
                    this.sendCommand(cmdDeleteBuilding(this.playerTeam, this.selectedBuilding.id));
                } else {
        ...
        const hasVillager = this.selectedUnits.some(u => u.isVillager && u.team === this.playerTeam);
        ...
        const cycleBuilding = (bType: BuildingType): boolean => {
            ...
            const buildings = this.entityManager.buildings.filter(b => b.team === this.playerTeam && b.type === bType && b.alive);
    ...
    private finishSelection(): void {
        ...
        if (Math.abs(dx) < 10 && Math.abs(dy) < 10) {
            ...
            const myUnits = this.selectedUnits.filter(u => u.team === this.playerTeam && u.alive);
            if (myUnits.length > 0) {
                // Check if clicking on an enemy unit → attack it
                const enemyUnit = this.entityManager.findEnemyUnitAt(wx, wy, this.playerTeam);
                ...
                const enemyBuilding = this.entityManager.findEnemyBuildingAt(wx, wy, this.playerTeam);
                if (enemyBuilding) {
                    for (const u of myUnits) {
            ...
            const unit = this.entityManager.findUnitAt(wx, wy, this.playerTeam);
            if (unit) {
                unit.selected = true;
            ...
            const building = this.entityManager.findBuildingAt(wx, wy, this.playerTeam);
            if (building) {
            ...
            const units = this.entityManager.findUnitsInRect(
                this.boxStartX, this.boxStartY, dx, dy, this.playerTeam
            );
    ...
    private handleRightClick(e: MouseEvent): void {
        ...
        // --- RALLY POINT LOGIC ---
        if (this.selectedBuilding && this.selectedBuilding.team === this.playerTeam) {
            // Check if building can train units (e.g. Barracks, Stable, ArcheryRange, TC)
            // Or just check if BUILDING_DATA[type].trainable exists
            const buildingData = BUILDING_DATA[this.selectedBuilding.type];
            if (buildingData && buildingData.trainable && buildingData.trainable.length > 0) {
                // Determine if right click was on a resource or enemy for smart rallying
                // (For now, just set exact coordinate)
                if (this.isMultiplayer && this.sendCommand) {
                    // Requires cmdSetRally from NetworkCommands
                    import('../network/NetworkCommands').then(mods => {
                        if (mods.cmdSetRally) {
                            this.sendCommand!(mods.cmdSetRally(this.playerTeam, this.selectedBuilding!.id, world.x, world.y));
                        }
                    });
                } else {
                    this.selectedBuilding.rallyX = world.x;
                    this.selectedBuilding.rallyY = world.y;
                }
                
                // Show rally point indicator (can reuse move indicator or make a custom one)
                this.commandIndicators.push({ x: world.x, y: world.y, timer: 0.4, type: 'rally' });
                return;
            }
        }
        ...
        const myUnits = this.selectedUnits.filter(u => u.team === this.playerTeam);
        if (myUnits.length === 0) return;
        console.log(`🖱️ handleRightClick: isMP=${this.isMultiplayer}, hasSendCmd=${!!this.sendCommand}, team=${this.playerTeam}, units=${myUnits.length}`);

        const unitIds = myUnits.map(u => u.id);

        // Check if clicking NEAR enemy unit → ATTACK
        const enemyUnit = this.entityManager.findEnemyUnitAt(world.x, world.y, this.playerTeam);
        if (enemyUnit && !enemyUnit.isStealthed) {
            if (this.isMultiplayer && this.sendCommand) {
                this.sendCommand(cmdAttackUnit(this.playerTeam, unitIds, enemyUnit.id));
            } else {
                for (const u of myUnits) {
                    u.attackUnit(enemyUnit, true);
                    u.manualCommand = true;
        ...
        const enemyBuilding = this.entityManager.findEnemyBuildingAt(world.x, world.y, this.playerTeam);
        if (enemyBuilding) {
            if (this.isMultiplayer && this.sendCommand) {
                this.sendCommand(cmdAttackBuilding(this.playerTeam, unitIds, enemyBuilding.id));
            } else {
                for (const u of myUnits) {
                    u.attackBuilding(enemyBuilding, true);
                    u.manualCommand = true;
        ...
        if (resource) {
            ...
            if (this.isMultiplayer && this.sendCommand) {
                if (villagers.length > 0) {
                    this.sendCommand(cmdGather(this.playerTeam, villagers.map(u => u.id), resource.x, resource.y));
                }
                if (others.length > 0) {
                ...
                for (const u of villagers) {
                    u.gatherFrom(resource, () =>
                        this.entityManager.findNearestDropOff(u.x, u.y, resource.resourceType, this.playerTeam)
                    );
                    u.manualCommand = true;
                }
                if (others.length > 0) {
        ...
        // Check if clicking on friendly building
        const building = this.entityManager.findBuildingAt(world.x, world.y, this.playerTeam);
        if (building) {
            const villagers = myUnits.filter(u => u.isVillager);
            const others = myUnits.filter(u => !u.isVillager);

            if (!building.built) {
                // Clicked unbuilt building → BUILD
                if (this.isMultiplayer && this.sendCommand) {
                    if (villagers.length > 0) {
                ...
                // Clicked BUILT building → Check if manual drop-off is possible
                const isDropOff = building.type === BuildingType.TownCenter || building.type === BuildingType.StoragePit || building.type === BuildingType.Granary;
                const carryingVillagers = villagers.filter(u => u.isCarrying);
                const nonCarryingVillagers = villagers.filter(u => !u.isCarrying);

                if (isDropOff && carryingVillagers.length > 0) {
                    if (this.isMultiplayer && this.sendCommand) {
                        this.sendCommand(cmdReturnResources(this.playerTeam, carryingVillagers.map(u => u.id), building.id));
                        if (nonCarryingVillagers.length > 0 || others.length > 0) {
                            const moveIds = [...nonCarryingVillagers.map(u => u.id), ...others.map(u => u.id)];
                            if (moveIds.length > 0) {
                                this.sendCommand(cmdMove(this.playerTeam, moveIds, world.x, world.y));
                            }
                        }
                    } else {
                        for (const u of carryingVillagers) {
                            u.returnResources(building);
                            u.manualCommand = true;
                        }
                        const moveUnits = [...nonCarryingVillagers, ...others];
                        if (moveUnits.length > 0) {
                            const spacing = 20;
                            const cols = Math.ceil(Math.sqrt(moveUnits.length));
                            for (let i = 0; i < moveUnits.length; i++) {
                                const row = Math.floor(i / cols);
                                const col = i % cols;
                                const tx = world.x + (col - cols / 2) * spacing;
                                const ty = world.y + (row - cols / 2) * spacing;
                                moveUnits[i].moveTo(tx, ty, undefined, this.tileMap);
                                moveUnits[i].manualCommand = true;
                            }
                        }
                    }
        ...
        if (this.isMultiplayer && this.sendCommand) {
            this.sendCommand(cmdMove(this.playerTeam, unitIds, world.x, world.y));
        } else {
            const spacing = 20;
            const cols = Math.ceil(Math.sqrt(myUnits.length));
            for (let i = 0; i < myUnits.length; i++) {
    ...
    private handleBuildPlace(keepBuildMode = false): void {
        ...
        if (this.isMultiplayer && this.sendCommand) {
            const builderIds = this.selectedUnits
                .filter(u => u.isVillager && u.team === this.playerTeam)
                .map(u => u.id);
            this.sendCommand(cmdBuildPlace(this.playerTeam, this.buildMode as any, this.buildGhostCol, this.buildGhostRow, builderIds));
        } else {
    ...
    renderOverlays(ctx: CanvasRenderingContext2D): void {
        ...
        for (const ind of this.commandIndicators) {
            ...
            if (ind.type === 'gather') {
                ...
                ctx.stroke();
            } else if (ind.type === 'rally') {
                const maxTime = 0.4;
                const progress = 1 - ind.timer / maxTime;
                const radius = Math.max(0, 5 + progress * 20); // Prevent negative radius
                const alpha = Math.max(0, 1 - progress);
                ctx.globalAlpha = alpha;
                ctx.strokeStyle = '#ffaa00'; // Orange/gold for rally
                ctx.lineWidth = 2.5;
                ctx.beginPath();
                ctx.arc(sx, sy, radius, 0, Math.PI * 2);
                ctx.stroke();
                
                // Draw a small flag pole inside
                ctx.beginPath();
                ctx.moveTo(sx, sy + 5);
                ctx.lineTo(sx, sy - 10);
                ...
                // Green move ring (fallback for 'move' and others)
                const maxTime = 0.3;
                // Safeguard against progress < 0 if timer is larger than maxTime somehow
                const progress = Math.min(1, Math.max(0, 1 - ind.timer / maxTime));
                const radius = Math.max(0, 5 + progress * 20);
                const alpha = Math.max(0, 1 - progress);
                ctx.globalAlpha = alpha;
