...
export type { HeroSkill } from "../config/HeroSkillsData";
...
export class Unit {
    ...
    x: number; y: number;
    renderX: number; renderY: number; // Visual position (lerped for smooth rendering)
    ...
    get isCarrying(): boolean {
        return this.totalCarriedAmount > 0;
    }

    // Returns the resource type with the highest amount being carried (for UI icon)
    get carriedType(): ResourceType | null {
        let maxType: ResourceType | null = null;
        let maxVal = 0;
        for (const [type, amt] of Object.entries(this.carriedResources)) {
            if (amt > maxVal) {
                maxVal = amt;
                maxType = type as ResourceType;
            }
        }
        return maxType;
    }
    ...
    animFrame = 0;
    animTimer = 0;
    frozenUntil = 0; // timestamp when freeze effect ends (for visual overlay)
    idleTimer = 0;
    facingRight = true;
    ...
    // Spartacus Lightning (public for UnitCombat and Renderer)
    lamaJupiterTimer = 0;
    // General passive storage
    passiveData?: any;
    ...
    update(
        ...
        // Bear roar — BearRider growls periodically while alive
        if (this.type === UnitType.BearRider && this.team === 0) {
            audioSystem.playBearRoar(this.x, this.y);
        }
        ...
        switch (this.state) {
            ...
            case UnitState.Moving:
                ...
                // Horse gallop sound for cavalry units
                if (isCavalryType(this.type) && this.team === 0) {
                    audioSystem.playHorseGallop(this.x, this.y);
                }
    ...
    // ---- Combat & Abilities Update ----
    private getCmbStrategy() {
        return CombatStrategyFactory.getStrategy(this.type);
    }

    // ---- Passive Ability (delegated to CombatStrategy) ----
    private applyPassiveIdle(dt: number, particles: any, tileMap?: TileMapRef): void {
        const strategy = this.getCmbStrategy();

        // Passive Skills (Auras / Timed buffs / Custom passive mechanics)
        if (strategy) {
            strategy.applyPassiveIdle(this, dt, particles);
            if (strategy.updatePassive) {
                const getNearbyUnits = (x: number, y: number, r: number) => {
                    return tileMap ? tileMap.getAllUnits().filter(u => Math.hypot(u.x - x, u.y - y) <= r) : [];
                };
                strategy.updatePassive(this, dt, particles, getNearbyUnits);
            }
        }
    }
