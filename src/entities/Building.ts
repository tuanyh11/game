...
export class Building {
    ...
    advanceBuild(dt: number, particles: ParticleSystem): boolean {
        ...
        if (this.buildProgress >= this.buildTime) {
            ...
            audioSystem.playSFXWithPitch('/musics/universfield-game-level-complete-143022_soLe8QkZ.mp3', 0.3, 1.0, cx, cy);
            return true;
        }
        return false;
    }
    ...
    cancelQueueItem(index: number, ps: PlayerState): boolean {
        ...
        // Refund cost
        if (ud.cost.food) ps.resources.food += ud.cost.food;
        if (ud.cost.wood) ps.resources.wood += ud.cost.wood;
        if (ud.cost.gold) ps.resources.gold += ud.cost.gold;
        if (ud.cost.stone) ps.resources.stone += ud.cost.stone;
        // Remove from queue
