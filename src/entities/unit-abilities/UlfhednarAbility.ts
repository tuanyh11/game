...
function fireLightningBolt(unit: Unit, target: Unit, particles: any): void {
    ...
    // Flash at sky origin
    particles?.emit({
        x: tx + (Math.random() - 0.5) * 20, y: skyY,
        count: 6, spread: 8,
    ...
    let boltX = tx + (Math.random() - 0.5) * 10;
    for (let i = 0; i <= boltSteps; i++) {
        const t = i / boltSteps;
        const by = skyY + (ty - skyY) * t;
        const zigzag = (1 - t) * (Math.random() - 0.5) * 24;
        boltX = tx + zigzag;
        ...
        if (i > 1 && i < boltSteps - 1 && Math.random() < 0.4) {
