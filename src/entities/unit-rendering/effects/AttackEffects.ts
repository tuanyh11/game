...
export function renderAttackWeapon(unit: Unit, ctx: CanvasRenderingContext2D, totalBob: number, swingCycle: number, swingPhase: number): void {
    switch (unit.type) {
        case UnitType.Spearman: {
            // Forward spear thrust (inspired by Dai Tong hero's animation)
            const thrust = Math.sin(swingPhase) * 11;
            const wobble = Math.cos(swingPhase * 2) * 0.1;

            ctx.translate(2 + thrust * 0.7, -6 + totalBob);
            ctx.rotate(Math.PI / 2 - 0.15 + wobble); // pointing forward

            // Spear shaft
            ctx.fillStyle = '#6a4a2a';
            ctx.fillRect(-1, -26, 2, 32);

            // Tassel / binding under the head
            ctx.fillStyle = '#aa3333';
            ctx.fillRect(-2, -24, 4, 3);
            ctx.fillStyle = '#daa520'; // small gold ring
            ctx.fillRect(-2, -21, 4, 1);

            // Spear head (sharp metal tip)
            ctx.fillStyle = '#ddd';
            ctx.beginPath();
            ctx.moveTo(0, -32);
            ctx.lineTo(-3, -24);
            ...
            // Center shine
            ctx.fillStyle = '#fff';
            ctx.fillRect(-0.5, -31, 1, 6);

            // Hand holding the spear
            ctx.fillStyle = '#e8c8a0';
            ctx.fillRect(-2, -6, 4, 4);

            // Forward thrust trail effect on extension
            if (thrust > 3) {
                ...
                ctx.beginPath();
                ctx.moveTo(0, -32);
                ctx.lineTo(0, -32 - thrust * 0.8);
                ctx.stroke();

                ctx.globalAlpha = 0.2;
                ctx.strokeStyle = '#ffaa44'; // slight fiery tip like Dai Tong
                ctx.lineWidth = 4;
                ctx.beginPath();
                ctx.moveTo(0, -30);
        ...
        case UnitType.HeroQiJiguang: {
            ...
            if (civ === CivilizationType.BaTu) {
                ...
                // 🗡️ Thích Kế Quang — Changdao heavy downward cleave
                ctx.save();
                ...
                if (cleavePhase > 0) {
                    ...
                    ctx.lineWidth = 3;
                    ctx.beginPath();
                    // Semi-circle trailing the blade
                    ctx.arc(0, 0, 26, -Math.PI / 2, -Math.PI / 2 + cleavePhase);
                    ctx.stroke();
                    ctx.globalAlpha = 1;
                }
                ...
                // ⚔ Musashi — Dual katana cross-slash with arms
                ctx.save(); // Protect outer state from translation and rotation below
                ...
                ctx.save();
                ctx.translate(8, -6 + totalBob); ctx.rotate(slashR);
                // Right arm
                ctx.fillStyle = '#e8c8a0'; // skin
                ctx.fillRect(-2, 4, 4, 6); // upper arm
                ...
                ctx.beginPath(); ctx.arc(0, -8, 16, slashR - 0.8, slashR + 0.2); ctx.stroke();
                ctx.globalAlpha = 1;
                ctx.restore();
                // Left arm + wakizashi
                ctx.save();
                ctx.translate(-6, -4 + totalBob); ctx.rotate(slashL);
                // Left arm
                ctx.fillStyle = '#e8c8a0';
                ...
                // 🗡 Spartacus — Gladius stab + shield bash with arms
                const stabOffset = swingCycle * 10;
                const shieldBash = Math.max(0, swingCycle) * 4;

                // Left arm + scutum shield (shield bash forward)
                ctx.save();
                ctx.translate(-8 + shieldBash, -4 + totalBob);
                ctx.rotate(-0.1);
                // Arm
                ctx.fillStyle = '#e8c8a0';
                ...
                ctx.fillStyle = '#e8c8a0';
                ctx.fillRect(-2, 7, 4, 3); // hand
                // Scutum shield (rectangular Roman)
                ctx.fillStyle = '#8b0000';
                ...
                // Right arm + gladius (diagonal slash arc)
                ctx.save();
                const slashAngle = swingCycle * 2.2 - 1.2; // swing from -1.2 to +1.0 rad
                ctx.translate(6, -8 + totalBob);
                ctx.rotate(slashAngle);
                // Arm — follows the sword swing
                ctx.fillStyle = '#e8c8a0';
                ctx.fillRect(-2, 0, 4, 6); // upper arm
                ctx.fillRect(-2, 4, 4, 5); // forearm
                ctx.fillStyle = '#5a1a1a';
                ctx.fillRect(-2, 7, 4, 3); // leather bracer
                ctx.fillStyle = '#e8c8a0';
                ctx.fillRect(-2, 9, 4, 3); // hand gripping
                // Gladius blade
                ctx.fillStyle = '#c8c8d0';
                ctx.fillRect(-1, -16, 3, 18);
                ctx.fillStyle = '#e0e0e8';
                ctx.fillRect(-1, -16, 1, 16); // edge shine
                // Blood groove
                ctx.fillStyle = '#aab';
                ctx.fillRect(0, -14, 1, 12);
                // Gold guard (cross-guard)
                ctx.fillStyle = '#ffd700';
                ...
                ctx.fillRect(-0.5, 2.5, 2, 3);
                // Gold pommel
                ctx.fillStyle = '#ffd700';
                ctx.fillRect(-1.5, 5.5, 4, 2);
                ctx.fillStyle = '#cc1111';
                ctx.fillRect(0, 6, 1, 1); // pommel ruby
                // Slash arc trail — visible during downswing
                if (slashAngle > -0.3) {
                    ctx.globalAlpha = 0.35;
                    ctx.strokeStyle = '#ddd';
                    ctx.lineWidth = 2;
                    ctx.beginPath();
                    ctx.arc(0, 0, 16, -Math.PI * 0.6, slashAngle - 0.2);
                    ctx.stroke();
                    // Blade flash at tip
                    ctx.fillStyle = '#fff';
                    ctx.globalAlpha = 0.5;
        ...
        case UnitType.Knight: {
            ...
            if (slashProgress > 0.3 && slashProgress < 0.9) {
                ...
                if (civ === CivilizationType.BaTu) {
                    ...
                    // Halberd sweep arc
                    ctx.strokeStyle = '#dd3333'; ctx.lineWidth = 3;
                    ctx.beginPath(); ctx.arc(0, -6 + totalBob, 18, arcStart - 0.7, arcStart + 0.3); ctx.stroke();
                    ctx.strokeStyle = '#ffcccc'; ctx.lineWidth = 1.5;
                    ctx.beginPath(); ctx.arc(0, -6 + totalBob, 16, arcStart - 0.5, arcStart + 0.2); ctx.stroke();
                } else {
        ...
        case UnitType.HeroQiJiguang: {
            ...
            if (slashP > 0.2) {
                ...
                if (civ4 === CivilizationType.DaiMinh) {
                    // 🐉 Triệu Tử Long — Forward spear thrust trail
                    const thrustAmt = slashP * 22;
                    ctx.strokeStyle = '#ff4422'; ctx.lineWidth = 3;
                    ctx.beginPath(); ctx.moveTo(4, -8 + totalBob);
                    ctx.lineTo(4 + thrustAmt, -8 + totalBob);
                    ctx.stroke();
                    ctx.strokeStyle = '#ffaa44'; ctx.lineWidth = 1.5;
                    ctx.beginPath(); ctx.moveTo(4, -6 + totalBob);
                    ctx.lineTo(4 + thrustAmt * 0.8, -6 + totalBob);
                    ctx.stroke();
                    // Impact point burst
                    ctx.fillStyle = '#ff6622';
                    ctx.beginPath(); ctx.arc(4 + thrustAmt, -8 + totalBob, 4 * slashP, 0, Math.PI * 2); ctx.fill();
                    ctx.fillStyle = '#fff';
                    ctx.beginPath(); ctx.arc(4 + thrustAmt, -8 + totalBob, 2 * slashP, 0, Math.PI * 2); ctx.fill();
                } else {
