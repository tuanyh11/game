...
export function drawSwords_DaiMinh(unit: Unit, ctx: CanvasRenderingContext2D, age: number, bob: number, legOff: number, lvl: number, cv: CivColors): void {
    ...
    // ── BACK ARM (Right Arm holding Dao) ──
    ctx.save();
...
// ============================================================
export function drawSpears_DaiMinh(unit: Unit, ctx: CanvasRenderingContext2D, age: number, bob: number, legOff: number, lvl: number, cv: CivColors): void {
    ctx.save();
    ...
    // Qiang Thrust Animation Logic
    let bodyRot = 0;
    let spearAngle = 0;
    let spearOffset = { x: 0, y: 0 };
    let rightArmRot = 0; // Back arm 
    let leftArmRot = 0;  // Front arm 

    if (attackProgress > 0) {
        if (attackProgress < 0.3) {
            // Windup: Pull spear back, twist body
            ...
            // Strike: Fast lunge forward
            const t = (attackProgress - 0.3) / 0.3;
            const easeOut = 1 - Math.pow(1 - t, 3);
            bodyRot = -0.2 + easeOut * 0.4;
            spearOffset.x = -5 + easeOut * 14; // Lunge reach
            spearOffset.y = -2 + easeOut * 1;
            spearAngle = -0.1 + easeOut * 0.2;
            rightArmRot = -0.4 + easeOut * 0.8;
            leftArmRot = 0.2 - easeOut * 0.3;
        } else {
            // Recovery: Pull back into stance
            const t = (attackProgress - 0.6) / 0.4;
            const easeIn = t * t;
            bodyRot = 0.2 * (1 - easeIn);
            spearOffset.x = 9 * (1 - easeIn);
            spearOffset.y = -1 * (1 - easeIn);
            spearAngle = 0.1 * (1 - easeIn);
            rightArmRot = 0.4 * (1 - easeIn);
            leftArmRot = -0.1 * (1 - easeIn);
        }
    } else {
        // Form alignment / Qiang technique stance
        const breath = Math.sin(unit.animTimer * 2) * 0.05;
        bodyRot = breath;
        spearAngle = -0.1 + breath * 0.2; // Pointing forward, slight angle
        leftArmRot = -0.2;
    }
    ...
    ctx.rotate(Math.PI / 5 + rightArmRot * 0.5);
    ...
    // Wax Wood Pole (Baila Shu)
    ctx.fillStyle = '#6a4a2a'; // Lighter, flexible wood
    ...
    // Spear head & Tassel (Zhen/Ying)
    const shaftEnd = poleLength * 0.6;

    // Red Horsehair Tassel (Distracts enemies, stops blood)
    ctx.fillStyle = '#dd2222';
    ctx.beginPath();
    ctx.moveTo(shaftEnd, -1);
    ctx.quadraticCurveTo(shaftEnd - 3, -6, shaftEnd - 1, -8);
    ctx.quadraticCurveTo(shaftEnd + 1, -4, shaftEnd + 2, 0);
    ctx.fill();
    ctx.beginPath();
    ctx.moveTo(shaftEnd, 1);
    ctx.quadraticCurveTo(shaftEnd - 3, 6, shaftEnd - 1, 8);
    ...
    // Ming Qiang Blade (Leaf shaped, elegant)
    ctx.fillStyle = age >= 4 ? '#eeeeee' : '#cccccc';
    const bladeLen = age >= 3 ? 8 : 6;
    ctx.beginPath();
    ctx.moveTo(shaftEnd, 0);
    ctx.quadraticCurveTo(shaftEnd + 2, -2, shaftEnd + bladeLen, 0);
    ctx.quadraticCurveTo(shaftEnd + 2, 2, shaftEnd, 0);
    ctx.fill();
    ...
    ctx.save();
    ctx.translate(-4, -2 + bob);
    ctx.rotate(Math.PI / 3 + leftArmRot);
...
// ============================================================
export function drawArchers_DaiMinh(unit: Unit, ctx: CanvasRenderingContext2D, age: number, bob: number, legOff: number, lvl: number, cv: CivColors): void {
    ctx.save();
...
// ============================================================
export function drawKnight_DaiMinh(unit: Unit, ctx: CanvasRenderingContext2D, age: number, bob: number, moving: boolean, legOff: number, lvl: number, cv: CivColors): void {
    ctx.save();
    ctx.scale(0.85, 0.85);
