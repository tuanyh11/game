...
export abstract class BaseCombatStrategy implements ICombatStrategy {
    ...
    protected executeBuildingAttack(context: CombatContext, target: Building, dx: number, dy: number): void {
        ...
        const atkAngle = Math.atan2(dy, dx);
        const hitX = target.x + (Math.random() - 0.5) * target.tileW * TILE_SIZE * 0.4;
        const hitY = target.y + (Math.random() - 0.5) * target.tileH * TILE_SIZE * 0.3;
    ...
    protected calculateAttackSpeedModifier(unit: Unit): number {
        ...
        const isMeleeHero = unit.type === UnitType.HeroSpartacus || unit.type === UnitType.HeroMusashi || unit.type === UnitType.HeroRagnar || unit.type === UnitType.HeroQiJiguang;
