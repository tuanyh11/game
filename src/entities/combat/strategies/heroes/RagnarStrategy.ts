...
export class RagnarStrategy extends MeleeHeroStrategy {
    ...
    public castHeroSkill(unit: Unit, skillIndex: number, particles: ParticleSystem, findNearestEnemy?: (x: number, y: number, team: number, range: number) => Unit | null, findNearestEnemyBuilding?: (x: number, y: number, team: number, range: number) => import("../../../Building").Building | null): void {
        ...
        switch (sid) {
            ...
            case 'viking_w1': // Mjölnir Hạ Thần — 30 AOE 
                HeroSkillUtils.aoeDamage(unit, 30, aoeRange, 8, findNearestEnemy);
                particles.emit({ x: unit.x, y: unit.y + 4, count: 48, spread: 6, speed: [140, 320], angle: [0, Math.PI * 2], life: [0.2, 0.5], size: [3, 8], colors: ['#795548', '#a1887f', '#ffaa00', '#fff'], gravity: 30, shape: 'circle' });
                for (let i = 0; i < 6; i++) { const a = (i / 6) * Math.PI * 2; particles.emit({ x: unit.x + Math.cos(a) * 30, y: unit.y + Math.sin(a) * 18 + 4, count: 3, spread: 2, speed: [60, 140], angle: [a - 0.3, a + 0.3], life: [0.08, 0.2], size: [2, 6], colors: ['#88ccff', '#fff'], gravity: 0, shape: 'rect' }); }
                particles.emit({ x: unit.x, y: unit.y + 8, count: 20, spread: 28, speed: [60, 200], angle: [-Math.PI * 0.85, -Math.PI * 0.15], life: [0.4, 1.0], size: [3, 6], colors: ['#6B4226', '#8B5A2B', '#D2691E'], gravity: 130, shape: 'rect' });
                break;
            ...
            case 'viking_m0': { // Rune Thiên Lôi — 35 AOE 
                ...
                if (findNearestEnemy) { const hs = new Set<number>(); for (let i = 0; i < 18 && hs.size < 6; i++) { const e = findNearestEnemy(unit.x, unit.y, unit.team, aoeRange); if (e && !hs.has(e.id)) { hs.add(e.id); const os = e.speed; e.speed = 0; setTimeout(() => { if (e.alive) e.speed = os; }, 1000); } } }
            ...
            case 'viking_m2': { // Phong Ấn Băng — freeze 1 enemy 3s
                if (findNearestEnemy) {
                    ...
                    if (e) {
                        const os = e.speed; e.speed = 0; setTimeout(() => { if (e.alive) e.speed = os; }, 3000);
